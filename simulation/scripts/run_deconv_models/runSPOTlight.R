...
run_spotlight_proj <- function(projDir, outDir, col){
...
# }
  ...
  props.refH <- run_spotlight(mixtures = mixture, ref_count_fn = refH_count_fn, ref_meta_fn = refH_meta_fn, col=col)
...
# # outDir <- file.path(projDir, 'spotlight_minor_results/')
# # system.time(run_spotlight_proj(projDir2,outDir, 'bio_celltype'))
# # system.time(run_spotlight_proj(projDir2, 'bio_celltype_major'))
# 
# ### EXTERNAL REFERENCE CORRECTED & AUGMENTED
...
# mixture = t(as.matrix(read.csv(mixture_count_fn,row.names=1)))
# 
# mixture.props <- run_spotlight(mixture, ref_count_fn, ref_meta_fn, 'bio_celltype')
# 
# write.csv(mixture.props, file.path(projDir, 'spotlight_minor_results/estimated_proportions_external_corrected_augmented.csv'))
