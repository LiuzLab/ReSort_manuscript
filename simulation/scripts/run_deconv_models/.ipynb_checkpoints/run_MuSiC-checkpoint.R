...
runMusic <- function(mixture.eset, ref_count_fn, ref_meta_fn, col){
...
runMusic_project <- function(projDir, col='bio_celltype'){
  ...
  refH_count_fn <- file.path(curwd, 'single_cell_ref/MIST_hybrid_immuneB_count_per_10k.csv')
  ...
  # st = Sys.time()
  # props.refI <- runMusic(mixture.eset = mixture.eset, ref_count_fn = refI_count_fn, ref_meta_fn = refI_meta_fn, col='bio_celltype')
  # et = Sys.time()
  # print(et-st)
  # write.csv(props.refI, file.path(projDir, 'music_estimated_proportions_external_immune_only.csv'))
...
system.time(runMusic_project(projDir, col = 'bio_celltype'))
